...
@Service
@Slf4j
@RocketMQMessageListener(consumerGroup = "guava-group1", topic = "guava_hello_topic")
public class MqDelayTaskListener implements RocketMQListener<MessageExt>, RocketMQPushConsumerLifecycleListener {
    ...
    public void prepareStart(DefaultMQPushConsumer consumer) {
        try {
            consumer.subscribe("guava_hello_topic", "*");
        } catch (Exception e) {
            e.printStackTrace();
        }
        consumer.registerMessageListener((MessageListenerOrderly)(msgs, context) -> {
            log.info("guava_hello_topic消费消息---consume order message begin");
            try {
            ...
            log.info("guava_hello_topic消费消息---consume order message end");
